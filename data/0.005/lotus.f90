program swimming_plate
  ...
  ! -- Hyperparameters
    ...
    real, parameter    :: A = 0.1*L, St_d = 0.3, k_x=16.0, k_z=16.0, h_roughness=0.005
  ...
  ! -- Setup solver
    ...
    !
      time_loop: do while (flow%time<finish/f.and..not.there)
        ...
        if((t>(finish-1)/f).and.(mod(t,0.05/f)<dt)) call flow%write(geom, average=.true., threeD=.false., write_vtr=.false.)
        ! exit
      end do time_loop
