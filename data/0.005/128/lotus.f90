program swimming_plate
  ...
  ! -- Hyperparameters
    ...
    real, parameter    :: A = 0.1*L, St_d = 0.3, k_x=128.0, k_z=128.0, h_roughness=0.005
  ...
  ! -- Setup solver
    ...
    ! -- Setup the grid
      ...
      call xg(1)%stretch(n(1), -1.5*L, -.35*L, 2.*L, 5.5*L, h_min=4., h_max=18., prnt=root)
      call xg(2)%stretch(n(2), -1.5*L, -0.35*L, 0.35*L, 1.5*L, prnt=root)
      if(ndims==3) xg(3)%h = 4.
    !
    ...
    ! -- Initialise fluid
      ...
      call flow%velocity%e(3)%perturb(0.05,zero_ave=.true.)
      flow%time = 0
    !
    ...
    !
      time_loop: do while (flow%time<finish/f.and..not.there)
        ...
        write(9,'(f10.4,f8.4,4e16.8,4e16.8,4e16.8,4e16.8)')&
              t*f,dt,pforce,ppower,vforce,enstrophy_body,tke_body
        ...
        ! if((t>(finish-2)/f).and.(mod(t,0.1/f)<dt)) call flow%write(geom, write_vtr=.true.)
        ! exit
      end do time_loop
