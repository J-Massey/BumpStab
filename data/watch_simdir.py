...
def fluid_snap(sim_dir, fn, count):
    ...
    fsim.u_low_memory_saver(fn, count, f"{os.getcwd()}/data/0.001/128/uvp")
    fsim.v_low_memory_saver(fn, count, f"{os.getcwd()}/data/0.001/128/uvp")
    fsim.p_low_memory_saver(fn, count, f"{os.getcwd()}/data/0.001/128/uvp")

def body_snap(sim_dir, fn, count):
    bsim = ReadIn(sim_dir, 'bodAv', 4096, ext="vti")
    bsim.save_sdf_low_memory(fn, count, f"{os.getcwd()}/data/0.001/128/uvp")


# Specify the directory to monitor
directory_to_watch = f"{os.getcwd()}/data/0.001/128/128-save"
