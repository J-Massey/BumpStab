...
def plot_cp_smooth():
    ...
    pxs, ts = np.linspace(0, 1, phase_avg[0].shape[1]), np.linspace(0, 1, phase_avg[0].shape[0])
    ax.contour(pxs, ts, kappa(pxs, ts).T, levels=[2.5], colors='k', linewidths=0.5, linestyles='--')
...
def plot_difference_spectra(ts, pxs, body):
    ...
    dt = 4 / body[0].shape[0]  # Temporal step

    cc64 = np.fft.fft2(body[1]-body[0])
    cc64 = np.fft.fftshift(cc64)
    cc128 = np.fft.fft2(body[2]-body[0])
...
def plot_cp_diff_var(ts, pxs, ph_avg, instant):
    ...
    print(diff_smooth.mean(), diff_128.mean())
    ...
    divider = make_axes_locatable(ax[0])
    hax1 = horizontal_integral(fig, divider, inst_smooth.sum(axis=1))
    vax1 = vertical_integral(fig, divider, inst_smooth.sum(axis=2))
